...
int main(int argc, char** argv)
{
  ...
  //  fread(flip,sizeof(float),Data_Size,fp)
	...
	posit32_t dataFlip = convertFloatToP32(number[bitFlipIndex / 32]);
	uint32_t* ptr = (uint32_t*)&dataFlip;
	uint32_t tmp = (*ptr ^ (0x1L << (bitFlipIndex/ Data_Size)));
	//float *newVal = (float *)(&convertP32ToFloat(ui32_to_p32(tmp)));
	number[bitFlipIndex / 32] = (float)convertP32ToDouble(ui32_to_p32(tmp));
